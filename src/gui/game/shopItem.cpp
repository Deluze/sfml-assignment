...
ShopItem::ShopItem(TowerType towerType, sf::Text towerName, sf::Text goldText) : UIElement({100.f, 100.f}),
...
std::shared_ptr<Tower> ShopItem::getTower() {
    return std::make_shared<Tower>(m_towerType);
}

void ShopItem::draw(sf::RenderTarget &target, sf::RenderStates states) const {
